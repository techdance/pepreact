...
class ShowCommunicationPreferences extends React.Component {
  render() {
    ...
    const { language } = communicationPreferences;
            ...
            <div className="inner-wrap">
              ...
              <div className="box-middle">
                <div className="content-icon">
                  { Object.keys(language).map((keyName, index) => (
                    <p className="mb-1" key={index}>
                      <span className="icon-regular icon-comment-check"></span>{" "}
                      <span key={index} className="cl-gray">
                        {language?.primary?.LanguageName}
                        {language?.secondary?.LanguageName}
                        {language?.tertiary?.LanguageName}
                      </span>
                    </p>
                  ))}
                </div>
