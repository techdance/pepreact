...
class EditProfileContainer extends React.Component {
  ...
  const token = AuthService.getToken();
    ...
    let  areaofexpertiseObj = credentials?.certificates?.map( (data, index) => ({ [' areaofexpertise'+index]: data  })).reduce((a,b)=>(Object.assign(a, b)))
    let collaborateduserprofessionalbioSaveObj = {
      token :token ,
      createDate :todayDate,
      modifiedDate :todayDate,
      areaofexpertise1 : areaOfExpertise1,
      areaofexpertise2 : areaOfExpertise2,
