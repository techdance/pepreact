...
class EditProfileContainer extends React.Component {
 ...
 // this.props.getCollaboratedUserProfileimage({ params: { token: AuthService.getToken() }, dataObj: null });
  ...
  axios.all([thoughtSave, communicationPreferencesSave,collaboratedUsercredential,
    ...
    toastr.clear();
    if(responseZero.status){
      setTimeout(() => toastr.success(`Sucessfully  Save Profile`),300);
    } else {
      ToastContainer.warning({ title: "Profile",  message: responseZero.message , optionsOverride: true});
      setTimeout(() => toastr.warning(responseZero.message), 300);
    }
    ...
    ToastContainer.error({ title: "Profile",  message: " Unable to Save Profile", optionsOverride: true})
