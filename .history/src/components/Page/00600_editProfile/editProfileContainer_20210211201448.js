...
class EditProfileContainer extends React.Component {
  ...
  const token = AuthService.getToken();
    ...
    let membarshipObj = credentials?.professionalMemberships[0]?.map( (data, index) => ({ ['membership'+index]: data.name  })).reduce((a,b)=>(object.assign(a, b)))
    let certificate1Obj = credentials?.professionalMemberships[0]?.map( (data, index) => ({ ['membership'+index]: data.name  })).reduce((a,b)=>(object.assign(a, b)))
