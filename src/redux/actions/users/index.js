...
export const getProfilePercentageDetails = (params, comState) => {
  return async dispatch => {
    await  API({
      url:'getProfilePercentageDetails',
      method:'GET',
      params:params
    }).then(response => {

      dispatch({
        type: "GET_PROFILE_PERCENTAGE_DETAILS",
        data: response.data,
        params
      })
    })
...
export const getLanguageList = (params, comState) => {
  return async dispatch => {
    await  API({
     ...
     console.log(' languagelist response : ', response)   ;
      dispatch({
